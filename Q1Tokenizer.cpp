...
int main() {
    ...
    infp.open(".\\prg.in");
    if (!infp) {
        cout << "ERROR - cannot open file 'prg.in'\n";
        errors++;  // Increment error count if file fails to open
    } else {
        nextChar = ' ';  // Initialize nextChar for tokenizer
        Tokens nextToken = tokenizer();  // Start tokenizing the input
		do {
		nextToken = tokenizer();  // Parse using the statement function
		//if (errors > 10) break;       // Break loop if too many errors occur
	} while (nextToken != ENDFILE);    // Continue until the end of the file is reached

    }
    // Output total number of errors encountered
    cout << "Total number of errors: " << errors << std::endl;
    return 0;
}
